...
CREATE POLICY "Allow EduFam admins full access" ON admin_communications
    FOR ALL USING (
        EXISTS (
            ...
            AND user_profiles.role = 'edufam_admin'
        )
    );
...
-- Create a scheduled job to clean up expired communications (runs daily)
SELECT cron.schedule(
    'cleanup-expired-communications',
    '0 2 * * *', -- Daily at 2 AM
    'SELECT cleanup_expired_communications();'
);

-- Insert some sample communications for testing
INSERT INTO admin_communications (title, message, target_roles, priority, dismissible) VALUES
(
    'System Maintenance Notice',
    'Scheduled maintenance will occur on Saturday, January 15th from 2:00 AM to 4:00 AM. During this time, the system may be temporarily unavailable.',
    ARRAY['teacher', 'principal', 'school_owner', 'finance_officer', 'parent'],
    'high',
    true
),
(
    'New Feature: Enhanced Reporting',
    'We have launched enhanced reporting features for teachers and principals. Check out the new analytics dashboard for better insights into student performance.',
    ARRAY['teacher', 'principal'],
    'medium',
    true
),
(
    'Fee Payment Reminder',
    'Please ensure all outstanding fees are settled by the end of the current term. Contact the finance office if you have any questions.',
    ARRAY['parent', 'finance_officer'],
    'medium',
    true
),
(
    'Welcome to EduFam',
    'Welcome to the new academic year! We hope you have a productive and successful year ahead.',
    ARRAY['teacher', 'principal', 'school_owner', 'finance_officer', 'parent'],
    'low',
    true
); 