-- Fix pgcrypto extension and create_admin_user function
-- Ensure pgcrypto extension is properly enabled
DROP EXTENSION IF EXISTS pgcrypto CASCADE;
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Update the create_admin_user function to handle password hashing properly
...
BEGIN
  ...
  -- Use crypt with explicit salt generation
  INSERT INTO auth.users (
    ...
    user_email,
    crypt(user_password, gen_salt('bf', 8)),
    now(),
    now(),
    now(),
    jsonb_build_object(
