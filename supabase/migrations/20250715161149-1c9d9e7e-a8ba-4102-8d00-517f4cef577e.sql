-- Alternative password hashing approach
-- This provides a fallback if pgcrypto extension cannot be enabled

-- Create a simple password hashing function that doesn't rely on pgcrypto
CREATE OR REPLACE FUNCTION public.hash_password_simple(password text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    -- Use a simple hash approach as fallback
    -- In production, you should use proper password hashing
    RETURN encode(sha256(password::bytea), 'hex');
END;
$$;

-- Update the create_admin_user function to use alternative hashing if pgcrypto is not available
