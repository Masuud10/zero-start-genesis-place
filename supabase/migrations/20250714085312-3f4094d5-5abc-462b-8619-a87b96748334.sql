...
CREATE INDEX IF NOT EXISTS idx_students_school_id_active_count 
...
CREATE INDEX IF NOT EXISTS idx_profiles_school_id_role_active 
ON public.profiles(school_id, role) 
WHERE status = 'active';

-- 6. Add index for classes table (used in dashboard stats)
CREATE INDEX IF NOT EXISTS idx_classes_school_id_active 
ON public.classes(school_id) 
WHERE is_active = true;

-- 7. Add index for subjects table (used in dashboard stats)
CREATE INDEX IF NOT EXISTS idx_subjects_school_id_active 
