-- Simple approach: Just migrate admin users and update the get_current_user_role function
-- This will ensure all admin users work with the admin_users table
...
-- Step 2: Update admin user profiles to 'parent' role (doesn't require school_id)
-- This preserves foreign key constraints while separating admin roles
UPDATE public.profiles 
SET role = 'parent', updated_at = now()
WHERE role IN ('edufam_admin', 'elimisha_admin', 'super_admin', 'support_hr', 'software_engineer', 'sales_marketing', 'finance');

-- Step 3: Update the get_current_user_role function to prioritize admin_users over profiles
...
-- Step 4: Create a helper function to check if current user is an admin
...
-- Step 5: Update the is_edufam_admin function to check for super_admin in admin_users
