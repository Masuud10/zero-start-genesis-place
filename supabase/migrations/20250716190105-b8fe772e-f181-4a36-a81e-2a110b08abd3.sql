-- Migration: 20250110000001-comprehensive-database-fixes-v2.sql
-- Fix all database functions and constraints for user/school creation

-- 1. Drop and recreate create_admin_user function with proper defaults
DROP FUNCTION IF EXISTS public.create_admin_user(text, text, text, text, uuid);
...
-- 2. Drop and recreate other functions that may have curriculum_type references
DROP FUNCTION IF EXISTS public.create_comprehensive_school(jsonb);
DROP FUNCTION IF EXISTS public.create_enhanced_school(text, text, text, text, text, text, text, text, text, text, integer, text, text, text, text, text, text);

-- Recreate create_comprehensive_school function
...
BEGIN
  ...
  IF school_data->>'director_email' IS NOT NULL AND school_data->>'director_name' IS NOT NULL THEN
    ...
    IF result->>'success' = 'true' THEN
      ...
      -- Update school with director_id if column exists
...
-- 3. Recreate create_enhanced_school function
