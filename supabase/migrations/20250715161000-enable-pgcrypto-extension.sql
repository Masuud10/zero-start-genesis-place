...
-- This extension provides the gen_salt() function used in create_admin_user

-- Enable the pgcrypto extension
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Verify the extension is enabled and gen_salt function exists
DO $$
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'pgcrypto')
    ) THEN
        RAISE EXCEPTION 'pgcrypto extension is not properly enabled. gen_salt function not found.';
    END IF;
    
    RAISE NOTICE 'pgcrypto extension is enabled and gen_salt function is available.';
END $$;

-- Update the create_admin_user function to ensure it uses the correct gen_salt function
...
BEGIN
  ...
  -- Use pgcrypto.gen_salt explicitly to ensure it's found
  INSERT INTO auth.users (
    ...
    user_email,
    crypt(user_password, pgcrypto.gen_salt('bf')),
    now(),
    now(),
    now(),
    jsonb_build_object(
