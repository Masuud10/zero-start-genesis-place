...
-- First, check and drop any foreign key constraints or dependencies
-- Drop any foreign keys that reference principal_id
DO $$ 
DECLARE
    constraint_record RECORD;
BEGIN
    -- Find and drop foreign key constraints that reference principal_id
...
-- Now safely remove the unnecessary columns from schools table
...
-- Update the create_comprehensive_school function to work with the new schema
...
BEGIN
  ...
  -- Create the school with cleaned schema
  INSERT INTO public.schools (
    ...
    school_type, registration_number, year_established, term_structure,
    owner_name, curriculum_type, now(), now()
