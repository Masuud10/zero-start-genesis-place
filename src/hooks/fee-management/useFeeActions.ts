  ...
  const assignFeeToClass = useCallback(async (classId: string, feeData: Record<string, unknown>, isRetry = false) => {
    ...
    try {
      ...
      const feeRecords = students.map(student => ({
        ...
        class_id: classId,
        amount: feeAmount,
        category: String(feeData.category || 'General'),
        term: String(feeData.term || 'Term 1'),
        academic_year: String(feeData.academic_year || new Date().getFullYear()),
        due_date: String(feeData.due_date || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]),
        status: 'pending',
        paid_amount: 0,
        discount_amount: 0,
        late_fee_amount: 0,
