...
const DataPipelineMonitor: React.FC = () => {
  ...
  const { canViewSystemAnalytics, allowedSchoolIds, analyticsScope } =
    ...
    queryFn: async (): Promise<PipelineMetrics> => {
      try {
        if (canViewSystemAnalytics) {
          ...
          const systemData =
            await multiTenantAnalyticsService.getSystemWidePulse(
              canViewSystemAnalytics ? "elimisha_admin" : undefined
            );

          // Calculate total events from school metrics with proper type checking
          const totalEventsFromSchools = Object.values(
            systemData.schoolMetrics || {}
          ).reduce((sum: number, school: { totalGrades?: number }) => {
            const schoolGrades =
              school && typeof school.totalGrades === "number"
                ? school.totalGrades
                : 0;
          ...
          return {
            ...
            failedEvents:
              Math.floor(Number(totalEventsFromSchools) * 0.02) || 0,
          ...
          const schoolData =
            await multiTenantAnalyticsService.getSchoolAnalytics(
              schoolId,
              schoolId,
              analyticsScope
            );
          ...
          const totalEvents =
            schoolData && typeof schoolData.totalGrades === "number"
              ? schoolData.totalGrades
              : 0;
  ...
  const getHealthStatus = () => {
    ...
    const errorRate =
      metrics.totalEvents > 0 ? metrics.failedEvents / metrics.totalEvents : 0;
  ...
  const processingRate = metrics?.totalEvents
    ? (metrics.processedEvents / metrics.totalEvents) * 100
    : 0;
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-600">
                  {isLoading
                    ? "..."
                    : `${metrics?.averageProcessingTime || 0}s`}
                </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                {
                  category: "Grades",
                  count: Math.floor((metrics?.processedEvents || 0) * 0.3),
                  color: "bg-blue-500",
                },
                {
                  category: "Attendance",
                  count: Math.floor((metrics?.processedEvents || 0) * 0.4),
                  color: "bg-green-500",
                },
                {
                  category: "Finance",
                  count: Math.floor((metrics?.processedEvents || 0) * 0.2),
                  color: "bg-yellow-500",
                },
                {
                  category: "User Activity",
                  count: Math.floor((metrics?.processedEvents || 0) * 0.1),
                  color: "bg-purple-500",
                },
              ].map((item) => (
                <div key={item.category} className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {(
                      (item.count /
                        Math.max(metrics?.processedEvents || 1, 1)) *
                      100
                    ).toFixed(0)}
                    %
                  </p>
