...
const AnalyticsOverview = () => {
  ...
  const { data: financialData, isLoading: financialLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const totalIncome = incomeData?.reduce((sum, item) => sum + parseFloat(String(item.amount || 0)), 0) || 0;
      const totalExpenses = expensesData?.reduce((sum, item) => sum + parseFloat(String(item.amount || 0)), 0) || 0;
