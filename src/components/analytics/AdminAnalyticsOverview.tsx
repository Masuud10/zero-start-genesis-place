...
const AdminAnalyticsOverview = () => {
  ...
  const { data: schoolsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const schoolTypes = schools?.reduce((acc: any, school: any) => {
        let type = 'Primary'; // Default
        if (school.name?.toLowerCase().includes('secondary') || school.name?.toLowerCase().includes('high')) {
          type = 'Secondary';
        } else if (school.name?.toLowerCase().includes('college') || school.name?.toLowerCase().includes('university')) {
          type = 'College';
        }
      ...
      const schoolsChartData = Object.entries(schoolTypes || {}).map(([type, count]) => ({
        type,
        count
      }));
  ...
  const { data: trendsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const trendsPromises = months.map(async ({ month, startDate, endDate }) => {
        // Get grades count
            ...
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                  ...
                  labelLine={false}
                >
                  {schoolsData?.statusData?.map((entry: any, index: number) => (
