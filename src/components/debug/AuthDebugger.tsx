...
const AuthDebugger: React.FC = () => {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      // Test 2: Admin user fetch (if user is authenticated)
      if (user) {
      ...
      // Test 3: Auth state consistency
      results.authState = !isLoading && !error && !!user;
    } catch (err) {
      console.error("Auth debug tests failed:", err);
    }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">Current Auth State</span>
