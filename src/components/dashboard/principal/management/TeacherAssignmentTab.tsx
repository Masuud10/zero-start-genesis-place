    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Assign teachers to the classes and subjects they will teach.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                <div className="flex flex-col md:flex-row gap-4">
                    <Select onValueChange={(value) => setSelectedClassId(value)} value={selectedClassId || ''}>
                        ...
                        <SelectContent>
                            {loadingEntities ? <div className="p-4 text-center text-muted-foreground">Loading classes...</div> : classList.map(c => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                        </SelectContent>
                    </Select>
                </div>
                ...
                {selectedClassId && (
                    <>
                        <div className="p-4 border rounded-md space-y-4 bg-muted/40">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                                <Select onValueChange={setSelectedTeacherId} value={selectedTeacherId || ''}>
                                    ...
                                    <SelectContent>{teacherList.map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent>
                                </Select>
                                ...
                                <Button onClick={handleAssign} disabled={isAssigning || !selectedTeacherId || !selectedSubjectId}>
                        ...
                        <div>
                            <h3 className="font-semibold text-md mb-2">Current Assignments for {classList.find(c => c.id === selectedClassId)?.name}</h3>
                            <div className="border rounded-md">
                                {loadingAssignments ? (
                                    <div className="flex justify-center items-center p-8"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>
                                ) : (
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                            ...
                                            {assignments && assignments.length > 0 ? assignments.map(a => (
                                                <TableRow key={a.id}>
                                                    ...
                                                    <TableCell className="text-right">
                                                        <Button variant="ghost" size="icon" onClick={() => handleUnassign(a.id)} disabled={isAssigning}>
                                                            <Trash2 className="h-4 w-4 text-destructive" />
                                                        </Button>
                                                    </TableCell>
                                                </TableRow>
                                            )) : (
                                                <TableRow><TableCell colSpan={3} className="text-center h-24 text-muted-foreground">No teachers assigned to this class yet.</TableCell></TableRow>
                                            )}
                                        </TableBody>
                                    </Table>
                                )}
