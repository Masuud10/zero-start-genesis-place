...
interface EnhancedGradingSheetWithExportProps {
  classId: string;
  term: string;
  examType: string;
...
export const EnhancedGradingSheetWithExport: React.FC<
  EnhancedGradingSheetWithExportProps
> = ({
  classId,
  term,
  examType,
  ...
  const loadGradingData = useCallback(async () => {
    if (!schoolId || !classId || !term || !examType) {
      console.log("❌ EnhancedGradingSheet: Missing required parameters", {
        schoolId: !!schoolId,
        classId: !!classId,
        term: !!term,
        examType: !!examType,
    ...
    try {
      setLoading(true);
      console.log("🎓 EnhancedGradingSheet: Loading data for", {
        classId,
        term,
        examType,
        curriculumType,
