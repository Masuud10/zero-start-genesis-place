...
const BulkGradingModal: React.FC<BulkGradingModalProps> = ({ onClose }) => {
  ...
  const fetchClassData = useCallback(async () => {
    ...
    setLoading(true);
    let subjectQuery = supabase.from('subjects').select('*').eq('class_id', selectedClass);
  ...
  const handleSubmit = async () => {
    if (!selectedClass || !selectedTerm || !selectedExamType) {
        toast({ title: "Missing Info", description: "Please select a class, term, and exam type.", variant: "destructive"});
        return;
    }
    setLoading(true);
    ...
    if (gradesToUpsert.length === 0) {
        toast({ title: "No grades to submit", description: "Please enter at least one grade.", variant: "default"});
        setLoading(false);
        return;
    }
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Enter grades for multiple students and subjects at once. Select a class, term, and exam to begin.</DialogDescription>
        </DialogHeader>
        ...
        {initialLoading ? (
            ...
            <>
                ...
                {loading && !initialLoading && <div className="flex items-center justify-center p-4"><Loader2 className="h-6 w-6 animate-spin" /> <span className="ml-2">Loading data...</span></div>}

                {!canProceed && (
                    <div className="flex-grow flex items-center justify-center">
                        <Alert className="max-w-md">
                            <AlertDescription>Please make selections above to load the grading sheet.</AlertDescription>
                        </Alert>
                    </div>
                )}
                ...
                {canProceed && !loading && (students.length === 0 || subjects.length === 0) && (
                    <div className="flex-grow flex items-center justify-center">
                       <Alert variant="destructive" className="max-w-md">
                           <AlertDescription>No students or subjects found for the selected class. Please check class setup and subject assignments.</AlertDescription>
                       </Alert>
                   </div>
                )}
            </>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={loading || !canProceed}>
            {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Submitting...</> : 'Submit Grades'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
