...
const PrincipalReportsModal: React.FC<PrincipalReportsModalProps> = ({ onClose }) => {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (format === 'pdf') {
        ...
        mimeType = 'text/plain';
        fileName = `${selectedReport?.label.replace(/ /g, '_')}_${period.replace(/ /g, '_')}.txt`;
      } else if (format === 'excel') {
        // For Excel, create CSV format
        fileContent = generateCSVContent(reportData);
        mimeType = 'text/csv';
        fileName = `${selectedReport?.label.replace(/ /g, '_')}_${period.replace(/ /g, '_')}.csv`;
      } else {
        // For CSV
        fileContent = generateCSVContent(reportData);
        mimeType = 'text/csv';
      ...
      // Create and download the report with proper content
  ...
  const generateCSVContent = (data: any): string => {
    ...
    csv += `${data.reportType},${data.period},${new Date(data.generatedAt).toLocaleString()},${data.school},${data.generatedBy}\n\n`;
    
    if (data.data && data.data.classPerformance) {
      csv += 'Class,Students,Average\n';
      data.data.classPerformance.forEach((cls: any) => {
        csv += `${cls.class},${cls.students},${cls.average}\n`;
      });
    }
