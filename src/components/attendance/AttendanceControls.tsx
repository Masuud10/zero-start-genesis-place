    ...
    <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div className="flex gap-2">
                <Button onClick={onSaveAttendance} disabled={saving || loading || !selectedClass || stats.total === 0 || !!academicInfoError}>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        ...
                        Attendance rate: <span className="font-bold">{!isNaN(attendancePercentage) ? attendancePercentage.toFixed(1) : 0}%</span>
                    </CardDescription>
                </CardHeader>
                <CardContent className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    <StatCard icon={<Users className="h-6 w-6 text-blue-600" />} title="Total Students" value={stats.total} color="bg-blue-100" />
                    <StatCard icon={<UserCheck className="h-6 w-6 text-green-600" />} title="Present" value={stats.present} color="bg-green-100" />
                    <StatCard icon={<UserX className="h-6 w-6 text-red-600" />} title="Absent" value={stats.absent} color="bg-red-100" />
                    <StatCard icon={<Clock className="h-6 w-6 text-yellow-600" />} title="Late" value={stats.late} color="bg-yellow-100" />
                    <StatCard icon={<BadgeHelp className="h-6 w-6 text-purple-600" />} title="Excused" value={stats.excused} color="bg-purple-100" />
                </CardContent>
            </Card>
        )}
