...
interface GeneratedReport {
  ...
  report_type: string;
  generated_at: string;
  filters: any; // Using any for database Json type compatibility
  generated_by: string;
  school_id: string;
}
...
const EnhancedReportGeneration: React.FC<EnhancedReportGenerationProps> = ({
  schoolId,
}) => {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke(
        "generate_report",
        {
          body: {
            ...
            schoolId,
            filters: {
              term: termFilter === "all" ? undefined : termFilter || undefined,
              class:
                classFilter === "all" ? undefined : classFilter || undefined,
      ...
      const { error: saveError } = await supabase.from("reports").insert({
        ...
        filters: {
          term: termFilter === "all" ? undefined : termFilter || undefined,
          class: classFilter === "all" ? undefined : classFilter || undefined,
          academic_year: new Date().getFullYear().toString(),
        },
      });
      ...
      toast({
        ...
        description:
          "Report has been generated successfully and added to history.",
      });
  ...
  const downloadReport = async (reportId: string) => {
    try {
      ...
      link.href = url;
      link.download = `${data.report_type}_${new Date(
        data.generated_at
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <Select value={reportType} onValueChange={setReportType}>
                ...
                <SelectContent>
                  <SelectItem value="academic_performance">
                  ...
                  <SelectItem value="class_performance">
                    Class Performance
                  </SelectItem>
                  <SelectItem value="student_progress">
                    Student Progress
                  </SelectItem>
                </SelectContent>
              </Select>
