...
import { Examination, UpdateExaminationData } from "@/types/academic";
...
const EditExaminationModal: React.FC<EditExaminationModalProps> = ({
  open,
  onClose,
  onSuccess,
  examination,
}) => {
  const [formData, setFormData] = useState<UpdateExaminationData>({
    id: examination.id,
    ...
    coordinator_id: examination.coordinator_id || "",
  ...
  // Update form data when examination prop changes
  useEffect(() => {
    setFormData({
      id: examination.id,
      name: examination.name,
  ...
  const handleInputChange = (
    field: keyof UpdateExaminationData,
    value: string | string[]
  ) => {
    setFormData((prev) => ({
      ...prev,
              ...
              {classes.map((classItem) => (
                <div key={classItem.id} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    checked={formData.classes?.includes(classItem.id) || false}
